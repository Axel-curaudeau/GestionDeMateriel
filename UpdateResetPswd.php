...
<body>
    ...
    if(!isset($_POST["Mail"]))
    {
        header("Location: ".DOMAIN_URL."/OrgaEDT/ForgotPswd.php?alerte=noEmail");
        return;
    }
    ...
    if(count($res) == 0)
    {
        header("Location: ".DOMAIN_URL."/OrgaEDT/forgotPswd.php?alerte=EmailError");
        return;
    }
    
    if(strtotime(date('Y/m/d h:i:s')) - strtotime($res[0]['lastResetPswd']) < 86400) // < 24h 
    {
        header("Location: ".DOMAIN_URL."/OrgaEDT/forgotPswd.php?alerte=spamReset");
        return;
    }

    $resetPswd = substr(str_shuffle('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'), 1, 8);

    $sql = 'UPDATE OrgaEDT_Personne SET lastResetPswd = CURRENT_TIMESTAMP, resetPswd = :resetPswd WHERE Mail = :Mail';
    ...
    mail($_POST['Mail'],'OrgaEDT : Mot de passe temporaire',  'Mail : '.$_POST['Mail'].'<br />
    ...
    header("Location: ".DOMAIN_URL."/OrgaEDT/LoginPage.php?alerte=PswdReset");
